...
When reviewing all 9 docs (no `--target`), spawn parallel agents — one agent per doc. Each agent runs a **complete, self-contained review** of ONE doc — all per-doc topics, all exchanges, all iterations up to `--iterations` max, all adjudication, all edits. An agent is the same as running `iterate-references --target <doc>` on that doc alone.

1. **Build work list.** Identify all 9 Step 3 docs in scope. Log: "Reviewing N docs: architecture, authority, interfaces, ..."
2. **Spawn parallel agents.** One agent per doc, all spawned in parallel (use multiple Agent tool calls in a single message). Each agent receives the doc file, context files (system designs, design doc, glossary, other Step 3 docs as read-only context, ADRs, known issues), review config, and full topic/adjudication instructions.
3. **Collect results.** As agents complete, log progress: "architecture.md — Issues: Y accepted, Z rejected (N of M complete)"
4. **Cross-doc consistency check.** After ALL per-doc agents complete, run the cross-topic consistency check across the full doc set. This evaluates cross-doc references, authority alignment, and consistency that only surfaces when reviewing the full set together.
